...
export default function TypingInterfaceV3({ text, currentPosition, onPositionChange }: TypingInterfaceProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-[80vh] flex flex-col items-center justify-center bg-white w-full"
    ...
    >
      <div className="w-full max-w-3xl mx-auto px-4">
        ...
        {/* Main typing area */}
        <div className="space-y-8 mb-8 w-full overflow-x-hidden">
          {/* Previous lines (completed) */}
          <div className="space-y-4 w-full">
            {lines.slice(0, currentLine).map((line, index) => (
              <div key={index} className="font-mono text-lg leading-relaxed text-gray-400 w-full whitespace-pre-wrap break-words">
                {line}
              </div>
            ))}
          </div>
          ...
          {!isCompleted && lines[currentLine] && (
            <div className="space-y-4 w-full">
              <div className="font-mono text-lg leading-relaxed w-full whitespace-pre-wrap break-words">
          ...
          <div className="space-y-4 w-full">
            {lines.slice(currentLine + 1, currentLine + 4).map((line, index) => (
              <div key={index} className="font-mono text-lg leading-relaxed text-gray-200 w-full whitespace-pre-wrap break-words">
                {line}
              </div>
            ))}
          </div>
